    ...
    public function bookToPdf(Book $book): string
    {
        ...
        $html = view('exports.book', [
            ...
            'locale'       => user()->getLocale(),
                ...
                $count = substr_count($html, "<div><p></p></div>");
                $position = 0;
                
                for($x =0 ; $x<$count; $x++){

                    $position = strpos($html, "<div><p></p></div>",$position + 10);
                    if(trim(substr($html,$position+20,42)) === "<div class=\"page-break\"></div>" ){
                        $html = substr_replace($html," ",$position+20,42);
                    } else {
                        Log::info("Mismatch in HTML For Blank Pages");
                    }
